...
Begin VB.Form Rpt_stdprogress 
   ...
   Begin VB.PictureBox Picture1 
      ...
      Height          =   750
      Left            =   -60
      Picture         =   "rptstdprogress.frx":0000
      ScaleHeight     =   690
      ScaleWidth      =   9645
      TabIndex        =   12
      Top             =   0
      ...
      Begin VB.Image Image1 
         Height          =   900
         Left            =   -120
         Picture         =   "rptstdprogress.frx":C897
         Stretch         =   -1  'True
         Top             =   0
         Width           =   9795
      End
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.ComboBox Combo2 
         Height          =   315
         ItemData        =   "rptstdprogress.frx":1973C
         Left            =   5970
         List            =   "rptstdprogress.frx":19746
         Style           =   2  'Dropdown List
         TabIndex        =   3
         Top             =   810
         Width           =   3045
   ...
   End
End
Attribute VB_Name = "Rpt_stdprogress"
...
Set rs = getdata("select classRoll From StudentAdmission where StudentId='" & Mid(List1, 1, 10) & "'" & _
 " and active_std=1 and serial_no=(select max(serial_no)  From StudentAdmission  where StudentId='" & Mid(List1, 1, 10) & "')")
If Not rs.EOF Then
...
Private Sub txtfields_LostFocus(Index As Integer)
 Select Case Index
   Case 0
      Dim rs As New ADODB.Recordset
      If Len(txtfields(0)) > 0 Then
      Set rs = getdata("select StudentName from studentinfo where studentid='" & txtfields(0).Text & "' and studentid not in(select studentid from studentAdmission where active_std=0)")
      If Not rs.EOF Then
             txtfields(1).Text = rs!StudentName
      Else
        MsgBox "Not an active Student", vbInformation, cmp
        txtfields(0).Text = ""
