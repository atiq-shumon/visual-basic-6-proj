...
Begin VB.Form frmIssueType 
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.Label Label1 
         ...
         Caption         =   "Issue Type Information Setup"
         BeginProperty Font 
            Name            =   "Garamond"
            Size            =   14.25
            Charset         =   0
...
Private Sub cmdDelete_Click()
            ...
            Set objRs = objcom.Get_RS("SELECT type_code from item_issue_type  WHERE (type_code= '" & txtfields(0) & "')", objmyCon)
...
Set RS = getdata("select max(type_code+1)from item_issue_type")
If Not RS.EOF Then
...
Private Sub save()
    ...
    cmd.CommandText = "{CALL s_U_d_item_issue_type(?,?,?,?,?,?)}"
...
Dim RS As New ADODB.Recordset
Set RS = objcom.Get_RS("SELECT type_code as Code ,type_name as Title,remarks  From item_issue_type order by type_code", objmyCon)
If Not RS.EOF Then
    i = 1
    With MSFlexGrid1
        Do Until RS.EOF
