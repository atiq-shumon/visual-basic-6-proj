...
Begin VB.Form frmSpecialIssue 
   ...
   Begin VB.Frame Frame1 
      ...
      Begin LVbuttons.LaVolpeButton CmdGenerate 
         ...
         EMBOSSS         =   16777215
         MPTR            =   0
         MICON           =   "frmSpecialIssueInfo.frx":0000
         ALIGN           =   1
         IMGLST          =   "(None)"
         IMGICON         =   "(None)"
         ICONAlign       =   0
   ...
   End
End
Attribute VB_Name = "frmSpecialIssue"
...
'    cmd.Parameters.Append Param5
'
'    Set Param6 = cmd.CreateParameter("param6", adVarChar, adParamInput, 4, Get_Code(CboSupplier))
'    cmd.Parameters.Append Param6
'
'    Set Param7 = cmd.CreateParameter("param7", adVarChar, adParamInput, 5, Get_Code(cboItem))
'    cmd.Parameters.Append Param7
...
'           End If
'
        Set objRs = objcom.Get_RS("SELECT ItemCode from IssueSub  WHERE (IssueId= '" & txtfields(0) & "') and ItemCode='" & cboItem & "' and PurchaseId='" & CboPurId & "'", objmyCon)
         
        If objRs.EOF Then
              Set objRs = objcom.Get_RS("SELECT (PurQty-(UsedQty+ReturnQty)) as PurQty from PurchaseSub  WHERE (PurId= '" & Trim(CboPurId) & "' and itemId='" & Trim(Get_Code(cboItem)) & "')", objmyCon)
            If Not objRs.EOF Then
   ...
   Select Case Index
          Case 0
              If Len(txtfields(0).Text) > 0 Then
               ...
               ShowFlexData
'
'               Set objRs = objcom.Get_RS("SELECT  IssueDate,IssueType,IssueTo,Remarks from IssueMain  WHERE (IssueId = '" & txtfields(0) & "')", objmyCon)
'
'              If Not objRs.EOF Then
'                 MaskEdBox2 = Format(objRs(0), "dd/mm/yy")
'                 CboPurType = select_issueto(objRs(1))
'                 'CboSupplier = Trim(select_issueto(objRs(2)))
        ...
        Case 0
         If Len(txtfields(0).Text) > 0 Then
             ...
             If Len(txtfields(0)) <> 0 Then
              ...
              End If
                   ShowFlexData
               Set objrs_local = objcom.Get_RS("SELECT  a.IssueDate,a.IssueType,b.type_name,a.Issueto,a.Remarks,a.indent_no,a.indent_date from IssueMain a,item_issue_type b WHERE (a.issueId= '" & txtfields(0) & "' and a.IssueType=b.type_code)", objmyCon)
...
Set RS = objcom.Get_RS("SELECT c.type_code, c.type_name,a.ItemCode  as Code ,b.item_name,a.PurchaseId,a.URate,a.Qty,a.TrackId   From issueSub a ,item_info b, item_type_info c where b.type_code = c.type_code and a.issueId='" & txtfields(0).Text & "' and to_number(b.item_code)=to_number(a.ItemCode) order by a.ItemCode ", objmyCon)
If Not RS.EOF Then
    i = 1
    With MSFlexGrid1
        Do Until RS.EOF
