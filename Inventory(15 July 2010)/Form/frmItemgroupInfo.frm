...
Begin VB.Form frmItemgroupInfo 
   BackColor       =   &H8000000B&
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.Label Label1 
         ...
         Caption         =   "Item Group Information Setup"
         BeginProperty Font 
            Name            =   "Arial"
            Size            =   14.25
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmItemgroupInfo"
...
Set RS = getdata("select max(TO_NUMBER(group_code)+1)from item_group_info")
If Not RS.EOF Then
    txtfields(0) = IIf(IsNull(RS(0)) = True, "01", Format(RS(0), "000"))
Else
    txtfields(0) = "01"
End If
...
Private Sub cmdSAVE_Click()
          If Len(txtfields(0)) = 0 Then
            ...
            If Not objRs.EOF Then
               MsgBox "Same Group Code  already Exists..Please Verify.", vbInformation, cmp
               cmdnew.SetFocus
               Exit Sub
            End If
...
Private Sub save()
    ...
    Set Param2 = cmd.CreateParameter("param2", adVarChar, adParamInput, 3, Format(Trim(txtfields(0)), "000"))
    cmd.Parameters.Append Param2
    
    Set Param3 = cmd.CreateParameter("param3", adVarChar, adParamInput, 100, Trim(txtfields(1)))
    ...
    cmd.CommandText = "{CALL  s_U_d_item_group_info(?,?,?,?,?,?,?)}"
...
load_type
Dim RS As New ADODB.Recordset
Set RS = getdata("select max (group_code+1)from item_group_info")
If Not RS.EOF Then
...
With MSFlexGrid1
    ...
    .Col = 0: .Text = " Code"
    .Col = 1: .Text = " Title"
    .Col = 2: .Text = " Category Code"
    .Col = 3: .Text = "Store Item Type"
    .Col = 4: .Text = " Remarks"
...
Private Sub load_type()
 Set objRs = objcom.Get_RS("SELECT type_name,type_code from item_type_info where cate_code='" & CategoryCode & "'", objmyCon)
 Cbocategory.Clear
 If Not objRs.EOF Then
    objRs.MoveFirst
    Do Until objRs.EOF
...
Set RS = objcom.Get_RS("SELECT a.group_code as Code ,a.group_name as Title,a.type_code, b.type_name, a.remarks  From item_group_info a,item_type_info b where a.type_code=b.type_code and a.type_code='" & Get_Code(Cbocategory) & "'order by a.group_code asc", objmyCon)
If Not RS.EOF Then
    i = 1
    With MSFlexGrid1
        Do Until RS.EOF
