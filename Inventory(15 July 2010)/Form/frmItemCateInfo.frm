...
Begin VB.Form frmItemCateInfo 
   ...
   Begin VB.PictureBox Picture1 
      ...
      Begin VB.Label Label1 
         ...
         Caption         =   "Store Category Information Setup"
         BeginProperty Font 
            Name            =   "Garamond"
            Size            =   14.25
            Charset         =   0
   ...
   End
End
Attribute VB_Name = "frmItemCateInfo"
...
Private Sub cmdDelete_Click()
       If Len(txtfields(0)) = 0 Then
          MsgBox "Item category Code Mandatory", vbInformation, App.title
     ...
     If Len(txtfields(1)) = 0 Then
                MsgBox "Item Category Name Mandatory", vbInformation, App.title
                txtfields(1).SetFocus
                Exit Sub
            End If
            
            Set objRs = objcom.Get_RS("SELECT cate_code from item_cate_info  WHERE (cate_code= '" & txtfields(0) & "')", objmyCon)
            If objRs.EOF Then
            ...
            Set objRs = objcom.Get_RS("SELECT cate_code from item_group_info  WHERE (cate_code= '" & txtfields(0) & "')", objmyCon)
            If Not objRs.EOF Then
               MsgBox "This category Code is already Used..Please Verify.", vbInformation, cmp
               cmdnew.SetFocus
               Exit Sub
            End If
...
Set RS = getdata("select max(cate_code+1)from item_cate_info")
If Not RS.EOF Then
...
Private Sub save()
    ...
    cmd.CommandText = "{CALL s_U_d_item_cate_info(?,?,?,?,?,?)}"
...
Dim RS As New ADODB.Recordset
Set RS = objcom.Get_RS("SELECT cate_code as Code ,cate_name as Title,remarks  From item_cate_info order by cate_code", objmyCon)
If Not RS.EOF Then
    i = 1
    With MSFlexGrid1
        Do Until RS.EOF
